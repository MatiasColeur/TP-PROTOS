...
char* printAddressPort(const struct addrinfo* aip, char addr[], size_t addrlen) {
    ...
    if (aip->ai_family == AF_INET) {
        ...
        strcpy(addr, addrAux);
        if (sinp->sin_port != 0) {
            snprintf(addr + strlen(addr), addrlen - strlen(addr), ": %d", ntohs(sinp->sin_port));
        }
        ...
        strcpy(addr, addrAux);
        if (sinp->sin6_port != 0)
            snprintf(addr + strlen(addr), addrlen - strlen(addr), ": %d", ntohs(sinp->sin6_port));
    } else
        strcpy(addr, "unknown");
    return addr;
}
...
int printSocketAddress(const struct sockaddr* address, char* addrBuffer) {
    ...
    if (inet_ntop(address->sa_family, numericAddress, addrBuffer, INET6_ADDRSTRLEN) == NULL)
        ...
        if (port != 0)
            snprintf(addrBuffer + strlen(addrBuffer), sizeof(addrBuffer) - strlen(addrBuffer), ":%u", port);
    }
    return 1;
}
